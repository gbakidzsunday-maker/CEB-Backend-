...
from app.deps import get_client_ip, log_security_event, enforce_rate_limit, get_current_candidate
from app.models import Candidate, Administrator, Parent, Course, CourseEnrollment
...
from app.schemas import (
    ...
    ChangePasswordRequest, CandidateMe,
)
...
def change_candidate_password(
    ...
    """Lets an authenticated candidate change their own password from their profile."""
    if not verify_password(payload.current_password, candidate.password_hash):
        log_security_event(
            db, "password_change_failed", candidate.matric_no, get_client_ip(request),
            "/auth/candidate/change-password", "Current password did not match",
        )
    ...
    db.commit()
    log_security_event(
        db, "password_changed", candidate.matric_no, get_client_ip(request),
        "/auth/candidate/change-password",
    )
    return None


@router.post("/candidate/me", response_model=CandidateMe)
def get_candidate_me(
    db: Session = Depends(get_db),
    candidate: Candidate = Depends(get_current_candidate),
):
    """
    Returns the authenticated candidate's profile, academic level, and
    enrolled courses — so the client never has to decode JWT claims
    itself to render a profile page.
    """
    courses = (
        db.query(Course)
    ...
    return CandidateMe(
        id=candidate.id, matric_no=candidate.matric_no, email=candidate.email,
        full_name=candidate.full_name, level=candidate.level, is_active=candidate.is_active,
        created_at=candidate.created_at, enrolled_courses=courses,
    )
