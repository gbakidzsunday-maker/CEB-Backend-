...
from app.models import (
    Candidate, Administrator, Parent, SecurityLog, Examination, CourseEnrollment,
    ExamProctoringFlag,
)
...
def require_not_terminated(db: Session, candidate_id: str, exam_id: str) -> None:
    """
    Server-authoritative anti-cheat gate. Blocks response submission
    and final exam submission once a candidate's proctoring strikes
    for this exam have reached the disqualification threshold — a
    client cannot bypass this by hiding or not reporting further
    strikes locally, since the flag is set the moment the 3rd strike
    is logged (see routers/exams.py::report_proctoring_event).
    """
    flag = (
        db.query(ExamProctoringFlag)
        .filter(ExamProctoringFlag.exam_id == exam_id, ExamProctoringFlag.candidate_id == candidate_id)
        .first()
    )
    if flag and flag.terminated:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="This exam attempt was terminated due to repeated proctoring violations.",
        )
