...
- **Admin student management**: admins can list/search all candidates,
  view a candidate's full profile (including their auto-enrolled
  courses), edit profile fields, deactivate/reactivate an account
  (blocks login without deleting data), manually clear a lockout, and
  view a candidate's exam results (with exam titles joined in).
...
app/
  ...
  routers/
    ...
    admin_students.py         /admin/students/... (admin-only student CRUD + results view)
...
All 42 tests should pass. Tests run against an isolated in-memory SQLite
...
`examinations`, an `is_active` column to `candidates`, and two new tables
(`courses`, `course_enrollments`). Since this project uses
`Base.metadata.create_all()` rather than Alembic migrations, it can only
*create* new tables — it won't add columns to tables that already exist.
If you have an existing `data/cbe.db` from before this change, delete it
(or point `DATABASE_URL` at a new file) before redeploying, or existing
rows will be missing the new columns.

## Admin: student profile & management — new endpoints

| Method | Path | Auth | Body / Query | Notes |
|---|---|---|---|---|
| GET | `/admin/students` | Admin | optional `?level=` and `?search=` (matches matric_no or full_name) | Returns every candidate with `is_active`, `is_locked` (computed), and `failed_login_attempts`. |
| GET | `/admin/students/{id}` | Admin | — | Same as above plus `enrolled_courses` (their auto-distributed courses). |
| PATCH | `/admin/students/{id}` | Admin | any of `{full_name, email, level}` | Partial update — send only changed fields. Changing `level` auto-enrolls the candidate into any existing course at the new level; it does **not** remove their existing enrollments from the old level, so enrollment history is preserved. `409` if the new email is already taken. |
| POST | `/admin/students/{id}/deactivate` | Admin | — | Blocks login (`403 "Account has been deactivated..."`) without deleting any of their data/results. |
| POST | `/admin/students/{id}/activate` | Admin | — | Reverses deactivation. |
| POST | `/admin/students/{id}/unlock` | Admin | — | Manually clears a lockout from repeated failed logins (same effect as waiting out `LOCKOUT_MINUTES`). |
| GET | `/admin/students/{id}/results` | Admin | — | That candidate's results, each with `exam_title` joined in (unlike `/results/me`, which doesn't include the title — see the frontend docs' note on this). |
